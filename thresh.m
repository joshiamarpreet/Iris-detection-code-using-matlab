...
function [ci,cp,out,time]=thresh(I,rmin,rmax)
scale=0.8;
tic
...
I=imresize(I,scale);
I=imcomplement(imfill(imcomplement(I),'holes'));  %imcomplement-- In the complement of a binary image, black becomes white and white becomes black. 
...
s=size(X,1);
for k=1:s
    if (X(k)>rmin) && (Y(k)>rmin) && (X(k)<=(rows-rmin)) && (Y(k)<(cols-rmin)) % rmin=50   rmax=200 rows=240 cols=320
